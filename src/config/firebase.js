...
try {
  ...
  // 2. Try FIREBASE_SERVICE_ACCOUNT_BASE64 (Render)
  ...
  // 3. Try GOOGLE_APPLICATION_CREDENTIALS / Application Default
  else {
    try {
  ...
  if (!admin.apps.length) {
    if (credential) {
      ...
      // Initialize with no credential (works in GCP) or throw to trigger mock
      // admin.initializeApp({ projectId: "medi-payment-reminder" }); 
      // db = admin.firestore();
